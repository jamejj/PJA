...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        var res = depts.Where(d => d.Loc == "Chicago".ToUpper()).Select(d => d.DeptNo).ToList();
        List<Emp> result = emps.Where(e => res.Contains(e.DeptNo) ).ToList(); 
    ...
    public void ShouldReturnEmployeesWithCommission()
    {
        var emps = Database.GetEmps();
         ...
         var result = emps.Select(e => new {e.EName,e.Comm}).Where(e => e.Comm != null); 
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var grades = Database.GetSalgrades();
         ...
         var result = emps.SelectMany(e => grades.Where(grade => e.Sal >= grade.Losal && e.Sal <= grade.Hisal)
                 .Select(grade => new { e.EName, grade.Grade })).ToList();;
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        var emps = Database.GetEmps();
         ...
         var result = emps.GroupBy(e => e.DeptNo).Select(a => new { DeptNo = a.Key, AvgSal = a.Average(e => e.Sal) }).ToList(); 
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        var emps = Database.GetEmps();
         ...
         var result = emps.Where(e => e.Sal > emps.Where(x => x.DeptNo == e.DeptNo).Average(x => x.Sal))
